...
def crawl_git(source_info: dict, console: Console = _console) -> str | None:
    ...
    console.print(f"[green]Repository ready — {count} Markdown files at {target}[/green]")
    return commit_hash
...
def _convert_rst_files(base: Path, console: Console) -> None:
    ...
    try:
        has_pandoc = subprocess.run(["pandoc", "--version"], capture_output=True).returncode == 0
    ...
    with Progress(
        ...
        for rst in rst_files:
            ...
            if has_pandoc:
                ...
                try:
                    md.write_text(_rst_to_markdown(rst.read_text("utf-8", errors="ignore")), "utf-8")
                except Exception:
                    pass
...
def _rst_to_markdown(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if nxt and re.match(r"^[=\-~^\"']{3,}$", nxt.strip()) and len(nxt.strip()) >= len(line.strip()):
...
def crawl_web(
    ...
    try:
        return asyncio.run(_crawl_web_async(source_info, max_pages, max_depth, delay, console))
...
async def _crawl_web_async(
    ...
    try:
        ...
        console.print(
            "[red]crawl4ai not installed.[/red] "
    ...
    console.print(f"[yellow]Crawling [bold]{url}[/bold] "
...
def _result_title(result, url: str) -> str:
    ...
    return urlparse(url).path.strip("/").split("/")[-1].replace("-", " ").title() or "index"
